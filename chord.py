...
from settings import SIZE, LOGSIZE, STABILIZE_INT, STABILIZE_RET,\
    FIX_FINGERS_INT, UPDATE_SUCCESSORS_INT, UPDATE_SUCCESSORS_RET,\
    N_SUCCESSORS

from network import read_from_socket, send_to_socket
...
class Local(object):
    ...
    # logging function
    def log(self, msg):
        logging.basicConfig(filename="/tmp/chord.log",
                            format="[%(levelname)s] %(message)s",
                            filemode="a+",
                            level=logging.INFO)
        logging.info(str(self.id()) + " : " + msg)
    ...
    def get_successors(self):
        ...
        return list(map(lambda node: (node.address_.ip, node.address_.port),
                   self.successors_[:N_SUCCESSORS-1]))
    ...
    def closest_preceding_finger(self, id):
        ...
        for remote in reversed(self.successors_ + self.finger_):
            if remote is not None and \
                    inrange(remote.id(), self.id(1), id) and remote.ping():
                return remote
        return self
    ...
    def run(self):
        ...
        while True:
            ...
            if command == 'get_successor':
                ...
                result = json.dumps((successor.address_.ip,
                                     successor.address_.port))
            if command == 'get_predecessor':
                # we can only reply if we have a predecessor
                if self.predecessor_ is not None:
                    predecessor = self.predecessor_
                    result = json.dumps((predecessor.address_.ip,
                                         predecessor.address_.port))
            ...
            if command == 'closest_preceding_finger':
                ...
                result = json.dumps((closest.address_.ip,
                                     closest.address_.port))
            if command == 'notify':
                npredecessor = Address(request.split(' ')[0],
                                       int(request.split(' ')[1]))
    ...
    def unregister_command(self, cmd):
        self.command_ = filter(lambda t: True if t[0] != cmd else False,
                               self.command_)
